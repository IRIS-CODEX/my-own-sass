...
`.trim();
  ...
  return (
    <div className="space-y-8 pb-8">
      {/* Top Creation Container - White & Yellow Glassy Card */}
      <div className="p-6 rounded-2xl bg-white/85 dark:bg-[#0c0e18]/85 backdrop-blur-md border border-yellow-300/40 dark:border-yellow-500/20 shadow-sm space-y-5">
        {/* Header with Mode Switcher */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-yellow-200/80 dark:border-[#1e2338] pb-4">
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-xl bg-yellow-400 text-slate-950 flex items-center justify-center shadow-xs font-bold border border-yellow-300 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="px-3.5 py-1.5 text-xs font-bold bg-white/90 dark:bg-[#15192c] hover:bg-yellow-100/70 text-slate-900 dark:text-yellow-300 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer border border-yellow-300/60 dark:border-[#1e2338]"
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-3 rounded-xl bg-yellow-50/60 dark:bg-[#101322] border border-yellow-300/50 dark:border-[#1e2338]">
          <div className="flex items-center gap-2 text-xs text-slate-900 dark:text-slate-200 font-bold shrink-0">
          ...
          <select
            ...
            className="w-full sm:w-auto px-3.5 py-1.5 rounded-lg text-xs font-bold bg-white dark:bg-[#131627] border border-yellow-300/70 dark:border-[#1e2338] text-slate-950 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-yellow-400/50 font-mono cursor-pointer"
          >
          ...
          <form onSubmit={handleSynthesize} className="space-y-4">
            <div className="relative">
              <textarea
                ...
                className="w-full p-4 text-xs font-medium text-slate-950 dark:text-slate-100 bg-yellow-50/30 dark:bg-[#070913] border border-yellow-300/70 dark:border-[#1e2338] rounded-xl focus:outline-hidden focus:ring-2 focus:ring-yellow-400/50 resize-y"
              />

              <button
                type="submit"
                disabled={isSynthesizing || !promptText.trim()}
                className={`absolute right-3 bottom-3 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all shadow-sm ${
                  promptText.trim() && !isSynthesizing
                    ? 'bg-yellow-400 hover:bg-yellow-300 text-slate-950 border border-yellow-300 cursor-pointer'
                    : 'bg-slate-300 dark:bg-slate-800 cursor-not-allowed text-slate-600 dark:text-slate-500'
                }`}
              >
            ...
            <div className="pt-1">
              <div className="text-xs font-bold text-slate-900 dark:text-slate-200 mb-2 flex items-center gap-1.5">
                <span>Quick Natural Language Presets:</span>
                <span className="text-[11px] font-medium text-slate-700 dark:text-slate-400">(Click to populate prompt)</span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {PROMPT_PRESETS.map((preset, idx) => (
                  <button
                    ...
                    className="px-2.5 py-1 text-xs rounded-lg bg-white hover:bg-yellow-50 dark:bg-[#131627] dark:hover:bg-[#1a2035] border border-yellow-300/60 dark:border-[#1e2338] text-slate-900 dark:text-slate-200 transition-all font-medium cursor-pointer"
                  >
                    + {preset.label}
                  </button>
                ))}
        ...
        {creationMode === 'structured' && (
          <form onSubmit={handleCreateStructuredRule} className="space-y-4">
            {/* Quick Templates Bar */}
            <div className="p-3 rounded-xl bg-yellow-50/70 dark:bg-[#101322] border border-yellow-300/60 dark:border-[#1e2338]">
              <div className="text-xs font-bold text-slate-900 dark:text-slate-200 mb-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {STRUCTURED_TEMPLATES.map((tmpl, idx) => (
                  <button
                    ...
                    className="px-2.5 py-1 text-xs rounded-lg bg-white dark:bg-[#14182b] border border-yellow-300/60 dark:border-[#1e2338] text-slate-900 dark:text-slate-200 font-medium hover:border-yellow-400 cursor-pointer"
                  >
                    + {tmpl.label}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs font-medium text-slate-950 dark:text-slate-100 bg-yellow-50/30 dark:bg-[#070913] border border-yellow-300/70 dark:border-[#1e2338] rounded-xl focus:ring-2 focus:ring-yellow-400/50"
                  required
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-xs font-bold text-slate-950 dark:text-slate-100 bg-white dark:bg-[#131627] border border-yellow-300/70 dark:border-[#1e2338] rounded-xl focus:ring-2 focus:ring-yellow-400/50"
                >
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs font-mono text-slate-950 dark:text-slate-100 bg-yellow-50/30 dark:bg-[#070913] border border-yellow-300/70 dark:border-[#1e2338] rounded-xl focus:ring-2 focus:ring-yellow-400/50"
                />
              </div>
              ...
              <div className="space-y-1.5 md:col-span-3">
                ...
                <textarea
                  ...
                  className="w-full p-3 text-xs font-medium text-slate-950 dark:text-slate-100 bg-yellow-50/30 dark:bg-[#070913] border border-yellow-300/70 dark:border-[#1e2338] rounded-xl focus:ring-2 focus:ring-yellow-400/50 resize-y"
                  required
                />
              </div>
            </div>
            ...
            <div className="flex justify-end pt-2">
              <button
                type="submit"
                className="px-5 py-2.5 rounded-xl text-xs font-bold bg-yellow-400 hover:bg-yellow-300 text-slate-950 border border-yellow-300 transition-all shadow-sm flex items-center gap-2 cursor-pointer"
              >
        ...
        {lastSynthesized && (
          <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs space-y-2 mt-3 animate-in fade-in">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 font-bold text-emerald-800 dark:text-emerald-300">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                <span>Rule Successfully Activated in Real-Time Enforcement Engine</span>
              </div>
              <span className="text-[10px] font-mono px-2 py-0.5 rounded font-bold bg-emerald-500/20 text-emerald-900 dark:text-emerald-200">
                GATEWAY ENFORCED
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5 pt-1">
              <div className="p-2.5 rounded-lg bg-white dark:bg-[#0e111e] border border-emerald-500/20">
                <span className="text-[10px] font-bold text-slate-800 dark:text-slate-400 block">Rule Name</span>
              ...
              <div className="p-2.5 rounded-lg bg-white dark:bg-[#0e111e] border border-emerald-500/20">
                <span className="text-[10px] font-bold text-slate-800 dark:text-slate-400 block">Risk Tier</span>
                <span className={`font-mono font-black ${
                  lastSynthesized.rule.riskLevel === 'GREEN' ? 'text-emerald-700 dark:text-emerald-400' :
                  lastSynthesized.rule.riskLevel === 'YELLOW' ? 'text-amber-800 dark:text-yellow-300' : 'text-rose-700 dark:text-rose-400'
                }`}>
              ...
              <div className="p-2.5 rounded-lg bg-white dark:bg-[#0e111e] border border-emerald-500/20">
                <span className="text-[10px] font-bold text-slate-800 dark:text-slate-400 block">Governed Tool</span>
                <span className="font-mono font-bold text-slate-950 dark:text-yellow-300 truncate block">
                  {lastSynthesized.rule.targetTool}
                </span>
              </div>

              <div className="p-2.5 rounded-lg bg-white dark:bg-[#0e111e] border border-emerald-500/20">
                <span className="text-[10px] font-bold text-slate-800 dark:text-slate-400 block">Condition Gate</span>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl bg-white/70 dark:bg-[#0c0e18]/70 backdrop-blur-md border border-yellow-300/40 dark:border-yellow-500/20">
          <div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* Category Filter */}
            <div className="flex items-center p-1 rounded-xl bg-yellow-100/60 dark:bg-[#131627] border border-yellow-300/60 dark:border-[#1e2338] text-xs">
            ...
            <input
              ...
              className="px-3.5 py-1.5 rounded-xl text-xs font-medium bg-white dark:bg-[#131627] border border-yellow-300/60 dark:border-[#1e2338] text-slate-950 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-yellow-400/50"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredRules.length === 0 ? (
            <div className="p-8 text-center rounded-2xl bg-white/85 dark:bg-[#0e111e] border border-yellow-300/40 dark:border-[#1e2338] text-xs font-medium text-slate-600 dark:text-slate-400">
            ...
            filteredRules.map((rule) => {
              ...
              if (rule.riskLevel === 'YELLOW') {
                riskBadgeStyle = 'bg-yellow-400/20 text-amber-950 dark:text-yellow-300 border-yellow-400/50';
              ...
              return (
                <div
                  ...
                  className={`p-5 rounded-2xl border transition-all ${
                    rule.isEnabled
                      ? 'bg-white/90 dark:bg-[#0c0e18]/90 backdrop-blur-md border-yellow-300/40 dark:border-yellow-500/20 shadow-sm'
                      : 'bg-yellow-50/20 dark:bg-[#070913]/60 border-yellow-200/50 dark:border-[#1e2338] opacity-60'
                  }`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-yellow-200/60 dark:border-[#1e2338]">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold border ${riskBadgeStyle}`}>
                        {rule.riskLevel}
                      </span>
                      <h4 className="font-bold text-xs sm:text-sm text-slate-950 dark:text-white">
                        {rule.ruleName}
                      </h4>
                      <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-yellow-100/80 dark:bg-[#15192c] text-amber-950 dark:text-yellow-300 border border-yellow-300/60 dark:border-[#1e2338]">
                        {rule.category}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-slate-700 dark:text-slate-300 font-mono">
                        Target: <span className="text-amber-950 dark:text-yellow-300 font-bold">{rule.agentName || 'All Agents'}</span>
                      </span>
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-rose-600 transition-colors cursor-pointer rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/20"
                        title="Delete rule"
                      >
                  ...
                  <div className="pt-3 space-y-3">
                    <div>
                      <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-slate-800 dark:text-slate-400 block mb-1">
                        Source Prompt Specification
                      </span>
                      <p className="text-xs font-medium text-slate-900 dark:text-slate-100 italic bg-yellow-50/50 dark:bg-[#131627] p-2.5 rounded-xl border border-yellow-200/80 dark:border-[#1e2338]">
                        "{rule.sourcePrompt}"
                      </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div className="p-3 rounded-xl bg-yellow-50/40 dark:bg-[#131627]/60 border border-yellow-200/80 dark:border-[#1e2338]">
                        <span className="text-[10px] font-mono font-bold text-slate-800 dark:text-slate-400 block">Governed Tool & Condition</span>
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          <code className="text-amber-950 dark:text-yellow-300 font-mono text-xs font-bold bg-yellow-200/60 dark:bg-yellow-400/10 px-1.5 py-0.5 rounded border border-yellow-300">
                            {rule.targetTool || 'general_action'}
                          </code>
                          <span className="text-slate-800 dark:text-slate-400 font-mono text-[10px] font-bold">IF</span>
                          <code className="text-slate-950 dark:text-slate-100 font-mono text-xs bg-white dark:bg-[#181d32] px-2 py-0.5 rounded border border-yellow-300/60 dark:border-[#1e2338]">
                            {rule.conditionExpression || 'true'}
                          </code>
                        </div>
                      </div>

                      <div className="p-3 rounded-xl bg-yellow-50/40 dark:bg-[#131627]/60 border border-yellow-200/80 dark:border-[#1e2338] flex items-start justify-between gap-2">
                        <div className="truncate">
                          <span className="text-[10px] font-mono font-bold text-slate-800 dark:text-slate-400 block">Injected System Guardrail</span>
                          <span className="text-slate-900 dark:text-slate-100 font-medium text-xs truncate block mt-0.5" title={rule.systemInstructionAddition}>
                            {rule.systemInstructionAddition}
                          </span>
                        </div>
                        <button
                          onClick={() => handleCopySnippet(rule.systemInstructionAddition, rule.id)}
                          className="p-1.5 hover:bg-yellow-200/60 dark:hover:bg-[#1a2035] rounded-lg text-slate-700 dark:text-yellow-400 transition-colors cursor-pointer shrink-0"
      ...
      {showPromptInspector && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-3xl rounded-2xl bg-white dark:bg-[#0c0e18] border border-yellow-400/60 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            {/* Modal Header */}
            <div className="p-4 sm:p-5 border-b border-yellow-200 dark:border-[#1e2338] flex items-center justify-between bg-yellow-50/60 dark:bg-[#121629]">
              <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="p-1.5 text-slate-500 hover:text-slate-950 dark:hover:text-white rounded-lg hover:bg-yellow-200/50 transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-3">
              <div className="flex items-center justify-between text-xs">
                <span className="font-mono font-bold text-slate-800 dark:text-yellow-300">
                  Total Active Directives: {promptRules.filter((r) => r.isEnabled).length}
                </span>
                <button
                  type="button"
                  onClick={() => handleCopySnippet(compiledSystemPrompt, 'modal-all')}
                  className="px-3 py-1 bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-bold rounded-lg transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
                >
              ...
              <pre className="p-4 rounded-xl bg-slate-950 text-yellow-300 font-mono text-xs leading-relaxed overflow-x-auto border border-yellow-500/40 shadow-inner">
